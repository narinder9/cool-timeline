...
if ( ! class_exists( 'CTP_AT_Meta_Box') ) :
...
class CTP_AT_Meta_Box {
  ...
  public function check_field_date() {
    ...
    if ( $this->has_field( 'date' ) && $this->is_edit_page() ) {
	  ...
	  wp_enqueue_script( 'clt-vali','https://ajax.aspnetcdn.com/ajax/jquery.validate/1.15.0/jquery.validate.js',array( 'jquery' ),false,true);
	      
      wp_enqueue_script( 'ctl-datepicker-validation', $plugin_path .'/js/jquery-ui/jquery.ui.datepicker.validation.min.js', array( 'jquery' ),false,true );
      ...
      if (is_array($meta) && count($meta) > 0  ){
         foreach ($meta as $me){
           ...
           if ( in_array( $field['fields'][0]['type'], array('image','file') ) )
            $mmm = $c +1 ;
           echo '<div class="at-repater-block"><table class="repeater-table" style="display: none;">';
           if ($field['inline']){
       ...
       $inc++;
        echo'<span class="clt_slide_name at-re-toggle">Slide '.$inc.'</span>
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px; min-height: 50px; width:50%;}
</style>';
  ...
  public function show_field_text( $field, $meta) {  
    $this->show_field_begin( $field, $meta );
    if($field['id']=="story_custom_link"){
    echo "<input type='text' class='at-text".( isset($field['class'])? ' ' . $field['class'] : '' )."' name='{$field['id']}' id='{$field['id']}' value='{$meta}' size='100' ".( isset($field['style'])? "style='{$field['style']}'" : '' )."/>";
    $this->show_field_end( $field, $meta );
    }else{
