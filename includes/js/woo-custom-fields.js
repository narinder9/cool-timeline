 ...
 */
 	...
 	setup_datepickers: function () {
 		if ( $( '.woo-input-calendar, .ct_input_calendar' ).length ) {
	 		$( '.woo-input-calendar, .ct_input_calendar' ).each(function () {
 ...
 */
 	...
 	setup_timefields: function () {
 		if ( $( '.ct_input_time_masked' ).length ) {
	 		$( '.ct_input_time_masked' ).each( function (){
 ...
 */
 	...
 	setup_upload_titletest: function () {
 		if ( $( 'input#title' ).length ) {
			...
			if(val == ''){
				$( '.ct_metabox_fields .button-highlighted' ).after( '<em class="ct_red_note">Please add a Title before uploading a file</em>' );
			};
		}
 	} // End setup_upload_titletest()
  
  }; // End cool_timelineCustomFields Object // Don't remove this, or the sky will fall on your head.

/**
 * Execute the above methods in the cool_timelineCustomFields object.
 *
 * @since 4.8.0
 */
	$(document).ready(function () {
	
		cool_timelineCustomFields.adjust_form_encoding();
		cool_timelineCustomFields.setup_datepickers();
		cool_timelineCustomFields.setup_timefields();
		cool_timelineCustomFields.setup_wordcounters();
		cool_timelineCustomFields.setup_image_selectors();
		cool_timelineCustomFields.setup_colourpickers();
		cool_timelineCustomFields.setup_upload_titletest();
		cool_timelineCustomFields.setup_field_tabber();
