  ...
  class BF_Admin_Page_Class {
    ...
    public function DisplayPage() {
      ...
      echo '<form method="post" name="'.apply_filters('apc_form_name', 'admin_page_class',$this).'" class="'.apply_filters('apc_form_class', 'admin_page_class',$this).'" id="'.apply_filters('apc_form_id', 'admin_page_class',$this).'" action="" enctype="multipart/form-data">
        ...
        echo '<h2><img src="'.COOL_TIMELINE_PLUGIN_URL.'/images/cooltimeline.png"> '.apply_filters('admin_page_class_h2',$this->args['page_title']).'</h2>'."\n".((isset($this->args['page_header_text']))? $this->args['page_header_text'] : '').' 
        </div>'."\n".'
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function show_field_size( $field, $meta ) {
		$html='';
    $this->show_field_begin( $field, $meta );

      if(empty($meta['width'])&& empty($meta['height'])){
            $meta = array(
            'width' => $field['std']['width'],
            'height' =>$field['std']['height'],
            );
        }else{
          if (!is_array($meta)){
          $meta = array(
         ...
         $html .="<div class='left_cont'><label for='{$field['id']}[width].'>Width</label>";
         $html .= "<input class='sizes' type='text' name='{$field['id']}[width]' id='{$field['id']}[width]' value='".$meta['width']."' size='8' /><small>px</small></div><div class='right_cont'>";  
          $html .="<label for='{$field['id']}[width].'>Height</label>";
         $html .= "<input class='sizes' type='text' name='{$field['id']}[height]' id='{$field['id']}[height]' value='".$meta['height']."' size='8' /><small>px</small></div>";  
   ...
   $faces = $this->google_font_list();
    ...
    foreach ( $faces as $key => $face ) {
     // $op .= "\t".'<option value="' . esc_attr( $key ) . '">' . esc_html( $face['name'] ) . '</option>'."\n";
		$op .= "\t".'<option value="'.$key.'">'.str_replace("+"," ",$face).'</option>'."\n";
   }
    if (isset($meta['face']))
...
//    $html .= $op. '</select>'."\n";
  ...
  public function addSizes($id,$args,$repeater=false){
    $new_field = array(
      'type' => 'size', 
      'id'=> $id,
      'std' => array(
